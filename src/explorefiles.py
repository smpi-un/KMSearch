    ...
    for root, _, files in os.walk(base_dir):
        for file in files:
            ...
            if ext in pdf_exts:
                ...
                extract_and_insert(ex1, input_file_path)
                ex2 = pdfocr.PdfOcrExtractor(model_path)
                extract_and_insert(ex2, input_file_path)
            if ext in excel_exts:
                ex = ExcelCellExtractor()
                extract_and_insert(ex, input_file_path)

def extract_and_insert(extractor: pdftext.Extractor, input_file_path: str):
    ...
    if reg_document_id is None:
        ...
        if extract_result is None:
            return
        # 短いものはリターン
        if len(extract_result.search_texts) <= 5:
            return
