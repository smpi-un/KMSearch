...
def ocr_pdf(input_file_path: str, model_path: str):
    ...
    file_data_id = models.document.insert_document(
        filehash.calculate_file_hash(input_file_path), os.path.getsize(input_file_path)
    )
    file_path_id = models.filepath.insert_file_path(
        file_data_id, input_file_path, False
    )
    for i, r in enumerate(res_for_json):
        page_id = models.page.insert_page_data(file_data_id, i)
